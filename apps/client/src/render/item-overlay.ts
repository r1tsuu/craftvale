...
const ITEM_MODEL = new Float32Array([
  0.94, 0, 0, 0,
  0, 0.94, 0, 0,
  0, 0, 0.94, 0,
  0, 0, 0, 1,
...
export class ItemOverlayRenderer {
  ...
  public constructor(private readonly nativeBridge: NativeBridge) {
    ...
    this.viewProjectionLocation = nativeBridge.gl.getUniformLocation(this.program, "uViewProjection");
  ...
  public render(
    ...
    for (const item of drawableItems) {
      ...
      const viewportHeight = Math.max(1, Math.round(item.height * framebufferHeight / windowHeight));
