...
export class PlayerController {
  ...
  public applyClientSettings(
    settings: Pick<ClientSettings, 'fovDegrees' | 'mouseSensitivity'>,
  ): void {
  ...
  private getWalkForwardVector(): Vec3 {
    return normalizeVec3(vec3(Math.cos(this.state.yaw), 0, Math.sin(this.state.yaw)))
  }
  ...
  private moveAxis(world: VoxelWorld, position: Vec3, axis: 'x' | 'y' | 'z', delta: number): Vec3 {
    if (delta === 0) {
      return position
    }
  ...
  private setMovementState(gamemode: PlayerGamemode, flying: boolean, resetTimers: boolean): void {
