...
const normalizeName = (value: string): string => value.trim().replace(/\s+/g, ' ').slice(0, 32)

const normalizeAddress = (value: string): string => value.trim().replace(/\s+/g, '').slice(0, 128)
...
export const isValidSavedServerName = (value: string): boolean => normalizeName(value).length > 0
...
export class JsonSavedServerStorage {
  ...
  public async saveServers(servers: readonly SavedServerRecord[]): Promise<void> {
    ...
    const persisted: PersistedSavedServers = {
      ...
      servers: sortServers(servers.filter((server) => server.id !== BUILTIN_LOCAL_SERVER_ID)),
    }
    await writeFile(
