...
import type { JoinedWorldPayload, LoadingProgressPayload, PlayerName } from '@craftvale/core/shared'
...
export class GameApp {
  ...
  private async tick(): Promise<void> {
    ...
    if (this.state.appMode === 'menu') {
      ...
      while (this.state.accumulator >= FIXED_TIMESTEP) {
        const stepInput = applyFixedStepInputEdges(input, this.state.pendingFixedStepInputEdges)
      ...
      overlayText = this.state.clientSettings.showDebugOverlay
        ? this.buildOverlayText(worldRuntime.world, x, y, z, yawDegrees, pitchDegrees, focusedBlock)
        : []
      const playHud = buildPlayHud(input.windowWidth, input.windowHeight, {
        ...
        pauseSettings:
          this.state.pauseScreen === 'settings'
            ? {
                settings: this.state.clientSettings,
                statusText: this.state.lastServerMessage || 'ADJUST SETTINGS AND GO BACK TO RESUME',
                busy: false,
              }
            : undefined,
        biomeName,
  ...
  private getCurrentBiomeName(worldX: number, worldZ: number): string | null {
    ...
    const biomeId = getBiomeAt(this.state.currentWorldSeed, Math.floor(worldX), Math.floor(worldZ))
  ...
  private async syncSavedServers(statusText = 'SELECT A MODE'): Promise<void> {
    const servers = await this.deps.savedServerStorage.loadServers()
    this.state.menuState = setMenuServers(setMenuStatus(this.state.menuState, statusText), servers)
  }
  ...
  private async joinServer(serverId: string): Promise<void> {
    ...
    if (!server) {
      this.state.menuState = setMenuStatus(this.state.menuState, 'SELECT A SERVER TO JOIN')
      return
    }
  ...
  private beginLoading(loadingState: Omit<WorldLoadingState, 'token'>): number {
  ...
  private getStartupChunkRadius(): number {
    return Math.max(0, Math.min(STARTUP_CHUNK_RADIUS, this.state.clientSettings.renderDistance))
  }
  ...
  private async createWorld(): Promise<void> {
    const worldName =
      this.state.menuState.createWorldName.trim() || suggestWorldName(this.state.menuState.worlds)
    ...
    try {
      ...
      const worlds = [...this.state.menuState.worlds, world].sort((left, right) =>
  ...
  private updateGame(input: ReturnType<NativeBridge['pollInput']>, deltaSeconds: number): void {
    const adapter = this.getClientAdapter()
    const worldRuntime = this.getWorldRuntime()
  ...
  private handlePlayOverlayInput(input: ReturnType<NativeBridge['pollInput']>): void {
    ...
    if (this.state.pauseScreen === 'settings' || this.state.pauseScreen === 'menu') {
      return
    }
