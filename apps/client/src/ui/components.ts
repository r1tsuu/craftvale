...
export const evaluateUi = (
  ...
  for (const component of components) {
    ...
    if (component.kind === 'slider') {
      ...
      const normalizedValue = Math.max(0, Math.min(1, (component.value - component.min) / range))
      ...
      if (dragging) {
        ...
        const nextValue =
          component.min + Math.round(pointerNormalized * steppedRange) * (component.step ?? 1)
        sliderChanges.push({
