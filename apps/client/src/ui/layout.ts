...
export const centerRect = (container: UiRect, width: number, height: number): UiRect => ({
...
export const stackY = (
  ...
  return items.map((item) => {
    const width =
      align === 'stretch'
        ? container.width
        : Math.min(item.width ?? container.width, container.width)
    const rect = {
    ...
    if (item.width === undefined) {
      width =
        autoItemsRemaining <= 1
          ? autoWidthRemainder
          : Math.floor(autoWidthRemainder / autoItemsRemaining)
