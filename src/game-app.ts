...
export class GameApp {
  ...
  private formatHotbarText(): string {
    return this.deps.clientWorldRuntime.inventory.slots
      .map((slot, index) => {
        const label = Blocks[slot.blockId].name.toUpperCase();
        const selected = this.deps.clientWorldRuntime.inventory.selectedSlot === index ? ">" : " ";
