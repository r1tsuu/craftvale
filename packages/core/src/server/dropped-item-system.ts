...
export class DroppedItemSystem {
  ...
  public async update(
    ...
    for (const [entityId, transform] of this.entities.droppedItemTransform.entries()) {
      ...
      const lifecycle = this.entities.droppedItemLifecycle.require(
        entityId,
    ...
    for (const player of players) {
      ...
      for (const entityId of nearbyEntityIds) {
        ...
        if (
          this.getDistanceSquared(player.state.position, transform.position) > PICKUP_RADIUS_SQUARED
        ) {
          continue
        }
  ...
  private async loadPersistedItems(): Promise<void> {
    ...
    for (const item of storedItems) {
      if (this.entities.registry.has(item.entityId)) {
        throw new Error(
          `Duplicate dropped item entity id "${item.entityId}" in world "${this.worldName}".`,
        )
      }
  ...
  private createDroppedItem(
    ...
    const velocity: [number, number, number] = [Math.cos(angle) * 1.1, 2.4, Math.sin(angle) * 1.1]
  ...
  private reindexItem(
    ...
    const previousKey = chunkKey(
      worldToChunkCoord(previousPosition[0], previousPosition[1], previousPosition[2]).chunk,
    )
    const nextKey = chunkKey(
      worldToChunkCoord(nextPosition[0], nextPosition[1], nextPosition[2]).chunk,
    )
