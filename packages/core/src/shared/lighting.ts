...
export type WorldTimePhase = 'sunrise' | 'day' | 'noon' | 'sunset' | 'night' | 'midnight'
...
export const advanceWorldTime = (time: WorldTimeState, deltaTicks: number): WorldTimeState => {
...
export const setWorldTimeOfDay = (timeOfDayTicks: number, dayCount = 0): WorldTimeState => {
  const wrappedTicks =
    ((Math.trunc(timeOfDayTicks) % WORLD_TICKS_PER_DAY) + WORLD_TICKS_PER_DAY) % WORLD_TICKS_PER_DAY
