...
export type LoggerColor = 'blue' | 'cyan' | 'green' | 'magenta' | 'red' | 'yellow' | 'gray'
...
export const createLogger = (prefix: string, color: LoggerColor): Logger => {
  const formattedPrefix = formatPrefix(prefix, color)

  const emit = (writer: (...args: unknown[]) => void, message: string): void => {
