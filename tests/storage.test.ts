...
test('binary world storage creates, persists, and deletes worlds', async () => {
  ...
  try {
    ...
    expect(
      getHotbarInventorySlots(loadedPlayerA!.inventory).find(
        (slot) => slot.itemId === ITEM_IDS.stone,
      )?.count,
    ).toBe(5)
    expect(
      getMainInventorySlots(loadedPlayerA!.inventory).find((slot) => slot.itemId === ITEM_IDS.sand)
        ?.count,
    ).toBe(9)
    ...
    expect(
      getHotbarInventorySlots(loadedPlayerB!.inventory).find((slot) => slot.itemId === ITEM_IDS.log)
        ?.count,
    ).toBe(9)
