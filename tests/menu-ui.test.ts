...
const getButtonActions = (components: ReturnType<typeof buildMainMenu>): string[] =>
  components.filter((component) => component.kind === 'button').map((component) => component.action)
...
test('play screen centers on play and quit actions', () => {
  ...
  expect(statusPanel?.rect.y).toBeGreaterThan(
    (quitButton?.rect.y ?? 0) + (quitButton?.rect.height ?? 0),
  )
})
...
test('worlds screen exposes world selection and create world navigation', () => {
  const actions = getButtonActions(
    buildMainMenu(1280, 720, createViewModel({ activeScreen: 'worlds' })),
  )
