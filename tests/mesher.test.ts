...
const getFaceUvs = (
  mesh: ReturnType<typeof buildChunkMesh>,
  faceIndex: number,
...
test("grass uses distinct top, bottom, and side atlas tiles", () => {
  ...
  expectFaceUsesTile(mesh, 0, "grass-side");
  expectFaceUsesTile(mesh, 2, "grass-top");
  expectFaceUsesTile(mesh, 3, "dirt");
});
...
test("dirt and stone reuse the same tile on every face", () => {
  ...
  expectFaceUsesTile(dirtMesh, 0, "dirt");
  expectFaceUsesTile(dirtMesh, 2, "dirt");
  ...
  expectFaceUsesTile(stoneMesh, 0, "stone");
  expectFaceUsesTile(stoneMesh, 2, "stone");
});
