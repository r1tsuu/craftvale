...
import { type WorkerInboundMessage, WorkerServerHost } from '../apps/client/src/worker/host.ts'
  ...
  try {
    ...
    for (let attempt = 0; attempt < 20; attempt += 1) {
      ...
      const sawJoinResponse = messages.some(
        (message) => message.kind === 'response' && message.type === 'joinWorld' && message.ok,
      )
    ...
    const loadingProgressEvents = messages.filter(isLoadingProgressEvent)
    ...
    expect(loadingProgressEvents.some((message) => message.payload.stage === 'ready')).toBe(true)
