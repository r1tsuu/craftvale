...
test("default inventory creates a full hotbar plus a starter glowstone stack", () => {
  ...
  expect(inventory.main[0]).toEqual({
    itemId: ITEM_IDS.glowstone,
    count: DEFAULT_INVENTORY_STACK_SIZE,
  });
  expect(
    inventory.main.slice(1).every((slot) => slot.itemId === ITEM_IDS.empty && slot.count === 0),
  ).toBe(true);
...
test("inventory normalization clamps counts and fills missing slots safely", () => {
  const inventory = normalizeInventorySnapshot({
    ...
    main: [{ itemId: ITEM_IDS.brick, count: 2 }],
    selectedSlot: 99,
  ...
  expect(inventory.hotbar).toHaveLength(9);
  expect(inventory.main).toHaveLength(MAIN_INVENTORY_SLOT_COUNT);
  expect(inventory.hotbar.find((slot) => slot.itemId === ITEM_IDS.grass)?.count).toBe(3);
  expect(inventory.hotbar.find((slot) => slot.itemId === ITEM_IDS.leaves)?.count).toBe(
    DEFAULT_INVENTORY_STACK_SIZE,
  );
  expect(inventory.hotbar[5]).toEqual({ itemId: ITEM_IDS.empty, count: 0 });
  expect(inventory.hotbar[8]).toEqual({ itemId: ITEM_IDS.empty, count: 0 });
  expect(inventory.main[0]).toEqual({ itemId: ITEM_IDS.brick, count: 2 });
...
test("adding items fills empty main-inventory stacks when hotbar stacks are full", () => {
  ...
  expect(result.inventory.main[0]).toEqual({ itemId: ITEM_IDS.glowstone, count: 64 });
  expect(result.inventory.main[1]).toEqual({ itemId: ITEM_IDS.grass, count: 5 });
});
...
test("inventory interaction picks up, places, and merges stacks", () => {
  ...
  expect(inventory.hotbar[0]).toEqual({ itemId: ITEM_IDS.empty, count: 0 });

  inventory = interactInventorySlot(inventory, "main", 1);
  expect(inventory.cursor).toBeNull();
  expect(inventory.main[1]).toEqual({ itemId: ITEM_IDS.grass, count: 64 });
  ...
  inventory = interactInventorySlot(inventory, "hotbar", 0);
  expect(inventory.hotbar[0]).toEqual({ itemId: ITEM_IDS.grass, count: 36 });
  expect(inventory.cursor).toBeNull();
});
